...
var IScroll = function(t, i, s) {
	...
	function n(s, e) {
		this.wrapper = "string" == typeof e.el ? i.querySelector(e.el) : e.el, this.indicator = this.wrapper.children[0], this.indicatorStyle = this.indicator.style, this.scroller = s, this.options = {
			listenX: !0,
			listenY: !0,
			interactive: !1,
			resize: !0,
		...
		h = function() {
			...
			o.getTime = Date.now || function() {
				...
				var r, h, a = t - i,
					l = s.abs(a) / e,
					c = 6e-4;
				return r = t + l * l / (2 * c) * (0 > a ? -1 : 1), h = l / c, o > r ? (r = n ? o - n / 2.5 * (l / 8) : o, a = s.abs(r - t), h = a / l) : r > 0 && (r = n ? n / 2.5 * (l / 8) : 0, a = s.abs(t) + r, h = a / l), {
					destination: s.round(r),
					duration: h
				}
			};
			var h = e("transform");
	...
	return e.prototype = {
		...
		_start: function(t) {
			if(!(1 != h.eventType[t.type] && 0 !== t.button || !this.enabled || this.initiated && h.eventType[t.type] !== this.initiated)) {
				this.options.preventDefault && !h.isAndroidBrowser && t.preventDefault();
		...
		_move: function(t) {
			if(this.enabled && h.eventType[t.type] === this.initiated) {
				...
				var i, e, o, n, r = t.touches ? t.touches[0] : t,
					a = this.hasHorizontalScroll ? r.pageX - this.pointX : 0,
					l = this.hasVerticalScroll ? r.pageY - this.pointY : 0,
					c = h.getTime();
		...
		_end: function(t) {
			if(this.enabled && h.eventType[t.type] === this.initiated) {
				...
				if(this.scrollTo(n, r), this.isInTransition = 0, this.initiated = 0, this.endTime = h.getTime(), !this.resetPosition(this.options.bounceTime)) {
					...
					if(this.options.momentum && 300 > o && (i = this.hasHorizontalScroll ? h.momentum(this.x, this.startX, o, this.maxScrollX, this.options.bounce ? this.wrapperWidth : 0) : {
							destination: n,
							duration: 0
						}, e = this.hasVerticalScroll ? h.momentum(this.y, this.startY, o, this.maxScrollY, this.options.bounce ? this.wrapperHeight : 0) : {
							destination: r,
							duration: 0
						...
						this.currentPage = u, n = u.x, r = u.y, c = this.options.snapSpeed || s.max(s.max(s.min(a, 1e3), s.min(a, 1e3)), 300), this.directionX = 0, this.directionY = 0, p = this.options.bounceEasing
					}
		...
		refresh: function() {
			if(this.wrapper.offsetHeight, this.wrapperWidth = this.wrapper.clientWidth, this.wrapperHeight = this.wrapper.clientHeight, this.scrollerWidth = this.scroller.offsetWidth, this.scrollerHeight = this.scroller.offsetHeight, this.maxScrollX = this.wrapperWidth - this.scrollerWidth, this.maxScrollY = this.wrapperHeight - this.scrollerHeight, this.hasHorizontalScroll = this.options.scrollX && this.maxScrollX < 0, this.hasVerticalScroll = this.options.scrollY && this.maxScrollY < 0, this.hasHorizontalScroll || (this.maxScrollX = 0, this.scrollerWidth = this.wrapperWidth), this.hasVerticalScroll || (this.maxScrollY = 0, this.scrollerHeight = this.wrapperHeight), this.endTime = 0, this.directionX = 0, this.directionY = 0, this.wrapperOffset = h.offset(this.wrapper), this._execEvent("refresh"), this.resetPosition(), this.options.snap) {
		...
		_initIndicators: function() {
			var t, i, s = this.options.interactiveScrollbars,
				e = ("object" != typeof this.options.scrollbars, "string" != typeof this.options.scrollbars);
			this.options.scrollbars ? (this.options.scrollY && (t = {
				...
				listenX: !1
			}, this.wrapper.appendChild(t.el)), this.options.scrollX && (i = {
				...
				resize: this.options.resizeIndicator,
				listenY: !1
			}, this.wrapper.appendChild(i.el))) : (t = this.options.indicators.length ? this.options.indicators[0] : this.options.indicators, i = this.options.indicators[1] && this.options.indicators[1]), t && (this.indicator1 = new n(this, t)), i && (this.indicator2 = new n(this, i)), this.on("refresh", function() {
		...
		_wheel: function(t) {
			if(this.enabled) {
				...
				if(clearTimeout(this.wheelTimeout), this.wheelTimeout = setTimeout(function() {
					...
					}, 400), t.preventDefault(), "wheelDeltaX" in t) i = t.wheelDeltaX / 120, s = t.wheelDeltaY / 120;
				else if("wheelDelta" in t) i = s = t.wheelDelta / 120;
				else {
					if(!("detail" in t)) return;
					i = s = -t.detail / 3
				}
				i *= this.options.mouseWheelSpeed, s *= this.options.mouseWheelSpeed, this.hasVerticalScroll || (i = s), e = this.x + (this.hasHorizontalScroll ? i * this.options.invertWheelDirection : 0), o = this.y + (this.hasVerticalScroll ? s * this.options.invertWheelDirection : 0), e > 0 ? e = 0 : e < this.maxScrollX && (e = this.maxScrollX), o > 0 ? o = 0 : o < this.maxScrollY && (o = this.maxScrollY), this.scrollTo(e, o, 0)
			}
		},
		_initSnap: function() {
		...
		goToPage: function(t, i, e, o) {
			o = o || this.options.bounceEasing, t >= this.pages.length ? t = this.pages.length - 1 : 0 > t && (t = 0), i >= this.pages[0].length ? i = this.pages[0].length - 1 : 0 > i && (i = 0);
		...
		_animate: function(t, i, s, e) {
			function o() {
				var u, d, m, f = h.getTime();
				return f >= p ? (n.isAnimating = !1, n._translate(t, i), n.resetPosition(n.options.bounceTime) || n._execEvent("scrollEnd"), void 0) : (f = (f - c) / s, m = e(f), u = (t - a) * m + a, d = (i - l) * m + l, n._translate(u, d), n.isAnimating && r(o), void 0)
			}
			var n = this,
				a = this.x,
				l = this.y,
		...
		_end: function(i) {
			this.initiated && (this.initiated = !1, i.preventDefault(), i.stopPropagation(), h.removeEvent(t, "touchmove", this), h.removeEvent(t, "MSPointerMove", this), h.removeEvent(t, "mousemove", this), this.moved && this.scroller._execEvent("scrollEnd"))
		},
